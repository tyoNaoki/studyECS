#ifndef ECS_EVENTDISPATCHERMT_HPP
#define ECS_EVENTDISPATCHERMT_HPP

#include "CallbackListMT.hpp"
...
namespace ECS {
	...
	namespace EVENT {
		...
		>
			class EventDispatcher_Multi;

		template<typename EventType, typename ReturnType, typename... Args, typename Policies>
		class EventDispatcher_Multi<EventType, ReturnType(Args...), Policies> {
		public:
			using CallBacks = CallbackList_Multi<ReturnType(Args...)>;
		...
		protected:
			std::unordered_map<EventType, CallBacks> listeners;
		};
